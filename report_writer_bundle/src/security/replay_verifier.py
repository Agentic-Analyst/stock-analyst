...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=None,
        help="Override the verifier prompt version (for example: v1_generic_quality or v2_injection_specific)",
    )
    parser.add_argument(
        "--verifier-mode",
        choices=["generic_quality", "injection_specific"],
        default=None,
        help="Override the verifier operating mode",
    )
    parser.add_argument(
        "--threshold",
        type=float,
    ...
    articles = [
        {
            ...
            "sanitizer_hits": list(payload.get("sanitizer_hits", [])),
        }
...
def replay_one_case(
    *,
    ...
    return {
        ...
        "injection_risk_confidence": verification.injection_risk_confidence,
        "reasons": verification.reasons,
        "reason_categories": verification.reason_categories,
        "suspicious_documents": verification.suspicious_documents,
        "high_impact_fields_at_risk": verification.high_impact_fields_at_risk,
...
def main() -> None:
    ...
    if args.verifier_prompt_version:
        config.verifier_prompt_version = args.verifier_prompt_version
    if args.verifier_mode:
        config.verifier_mode = args.verifier_mode
